...
def test_vllm_scaled_mm_replay(with_bias):
    ...
    bias = torch.zeros(48, device=a.device, dtype=torch.bfloat16) if with_bias else None
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX split reduction")
@pytest.mark.parametrize(
    "shape",
    [
        (95, 1024, 1025),
...
def _check_metax_tle_graph(shape, dtype, b_offset, a_offset=0, check_small=None):
    ...
    if (m, n, k) == (256, 37888, 3584) and hasattr(_backend, "_pick_aligned_stream"):
    ...
    with torch.cuda.stream(stream):
        ...
        for av, bv, sav, sbv in [
            ...
            torch.testing.assert_close(out.cpu(), value.expand(m, n), rtol=0, atol=0)
            assert (storage[m * n :] == 123).all()
    torch.cuda.current_stream().wait_stream(stream)


@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX narrow reduction")
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX wide streaming TLE")
...
def test_metax_tle_wide_stream(k, dtype):
    ...
    for row in range(0, m, 128):
        integer[row : row + 128] = (ac[row : row + 128].double() @ bc).to(torch.int64)
    ref = (integer.float() * sa.cpu() * sb.cpu().t()).to(dtype)
    torch.testing.assert_close(out.cpu(), ref, rtol=0, atol=0)
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX TLE grid ordering")
@pytest.mark.parametrize(
    "shape",
    [
        (m, n, k)
...
def test_metax_tle_stream_split_group(shape, offsets, dtype):
    ...
    b = (
        torch.randint(-128, 128, (n * k + bo,), device=a.device, dtype=torch.int8)[bo:]
        .view(n, k)
        .t()
    )
    sa = torch.randn(m, 1, device=a.device) * 0.001
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX zero-row padding")
@pytest.mark.parametrize(
    "shape",
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX reverse MMA chains")
@pytest.mark.parametrize(
    "shape",
    [(m, 14336, 4096) for m in (64, 65, 98, 127, 128, 256)]
    + [(98, 4096, 14336), (256, 3584, 18944), (2048, 2048, 2048), (8192, 1024, 4096)],
)
...
@pytest.mark.parametrize(
    "shape",
    [(m, n, k) for m in (64, 65, 98, 127, 128) for n, k in ((3584, 3584), (4096, 4096))]
...
@pytest.mark.parametrize("shape", [(1848, 1536, k) for k in (128256, 151936, 152064)])
@pytest.mark.parametrize("offsets", [(0, 0), (16, 128)])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
        ...
        def __getitem__(self, grid):
            def run(*args, **kwargs):
                ...
                if kwargs.get("stream_shared_mma_tile") == 256:
                    expected = (
                        args[0].data_ptr() % 128 == 0 and args[1].data_ptr() % 128 == 0
                    )
                    compiled = result[0]
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX small-N streaming")
@pytest.mark.parametrize(
    "shape",
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX square streaming")
@pytest.mark.parametrize(
    "shape,offsets",
    [
        ((4096, 4096, 4096), off)
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX K16 bias fallback")
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_metax_small_mma_k16_bias(dtype, monkeypatch):
    a = torch.randint(
        -128, 128, (1, 3, 3584), device=flag_gems.device, dtype=torch.int8
    )
    b = torch.randint(-128, 128, (3584, 3584), device=a.device, dtype=torch.int8).t()
    ...
    ref = (ref * sa.cpu() * sb.cpu() + bias.cpu().float()).to(dtype).view(1, 3, 3584)
    y = flag_gems.mm_w8a8_int8(a, b, sa, sb, dtype, bias)
    torch.testing.assert_close(y.cpu(), ref, rtol=0, atol=0)
    assert launches
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX split transpose")
@pytest.mark.parametrize(
    "shape,offsets",
    [
        ((98, n, k), off)
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX long-K grouping")
@pytest.mark.parametrize(
    "shape,offsets,dtype",
    [
...
@pytest.mark.skipif(flag_gems.vendor_name != "metax", reason="MetaX M8 K16 boundaries")
@pytest.mark.parametrize(
    "shape",
    [(m, n, k) for m in (7, 9) for n, k in ((3584, 3584), (4608, 3584), (14336, 4096))]
    ...
    class CheckedKernel:
        def __getitem__(self, grid):
            def launch(*args, **kwargs):
                ...
                result = original[grid](*args, **kwargs)
                compiled, _ = result
                assert getattr(compiled.metadata, "mma_unroll_count", None) == expected
