...
__aicore__ inline void WrapPos(AscendC::TBuffAddr &addr, uint32_t bytes, uint32_t offset, AscendC::TPosition pos)
{
    addr.dataLen = bytes;
...
__aicore__ inline void RunFixpipeFull(int64_t acc, __gm__ uint64_t *deq, __gm__ half *diag,
    __gm__ uint16_t *c, int32_t offM, int32_t offN, int32_t tileM, int32_t tileN, int32_t accStride,
    int32_t ldc, int32_t loadDeq, int32_t loadDiag, int32_t outBf16)
{
    if ASCEND_IS_AIV {
        return;
    }
    ...
    if (loadDeq == 2 && fullDeq) {
        ...
        CopyDeqToC1(deq + offN, kDeqC1Addr,
            static_cast<uint32_t>(nAlign));
    }

    const uint32_t deqOff =
        (fullDeq && (loadDeq == 2 || loadDeq == 0))
    ...
    WrapPos(accAddr, static_cast<uint32_t>(accStride) * static_cast<uint32_t>(nAlign) * sizeof(int32_t), accOff,
        AscendC::TPosition::CO1);
    accLocal.SetAddr(accAddr);

    AscendC::LocalTensor<half> tmpC1;
    AscendC::TBuffAddr tmpAddr;
    WrapPos(tmpAddr, static_cast<uint32_t>(accStride) * static_cast<uint32_t>(nAlign) * sizeof(half), kTmpC1Addr,
    ...
    WrapPos(diagC1Addr, static_cast<uint32_t>(accStride) * static_cast<uint32_t>(accStride) * sizeof(half),
        kDiagC1Addr, AscendC::TPosition::C1);
    diagC1.SetAddr(diagC1Addr);

    AscendC::LocalTensor<half> a2;
    AscendC::TBuffAddr a2Addr;
    WrapPos(a2Addr, static_cast<uint32_t>(accStride) * static_cast<uint32_t>(accStride) * sizeof(half), 0,
        AscendC::TPosition::A2);
    a2.SetAddr(a2Addr);

    AscendC::LocalTensor<half> b2;
    AscendC::TBuffAddr b2Addr;
    WrapPos(b2Addr, static_cast<uint32_t>(accStride) * static_cast<uint32_t>(nAlign) * sizeof(half), 0,
    ...
    WrapPos(scaledAccAddr, static_cast<uint32_t>(accStride) * static_cast<uint32_t>(nAlign) * sizeof(float), accOff,
        AscendC::TPosition::CO1);
...
extern "C" [aicore] __attribute__((always_inline)) void _mlir_ciface_fixpipe_vdeqf16(int64_t acc, int64_t deq,
    int64_t diag, int64_t c, int32_t offM, int32_t offN, int32_t tileM, int32_t tileN, int32_t accStride,
    int32_t ldc, int32_t loadDeq, int32_t loadDiag, int32_t outBf16)
{
    RunFixpipeFull(acc, reinterpret_cast<__gm__ uint64_t *>(deq), reinterpret_cast<__gm__ half *>(diag),
        reinterpret_cast<__gm__ uint16_t *>(c), offM, offN, tileM, tileN, accStride, ldc, loadDeq, loadDiag,
        outBf16);
}
#endif
