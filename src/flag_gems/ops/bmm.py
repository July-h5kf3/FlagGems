...
def bmm(
    ...
    if A.dtype == torch.int8 and runtime.device.vendor_name == "thead":
...
def fp8_einsum(
    ...
    if x.dtype in (
        ...
        if runtime.device.vendor_name != "nvidia":
            raise TypeError("PPU low-precision einsum requires INT8 inputs")
        from flag_gems.runtime.backend._nvidia.hopper.ops.fp8_einsum import (
            fp8_einsum as nvidia_fp8_einsum,
        )

        return nvidia_fp8_einsum(
            equation, x, xs, y, ys, block_size=block_size, output_dtype=output_dtype
        )
    ...
    if x.dtype == torch.int8:
        if runtime.device.vendor_name != "thead":
            raise TypeError("INT8 fp8_einsum is supported on PPU")
