...
from . import base, conftest, consts
...
def _einsum_low_precision_available():
    ...
    if flag_gems.vendor_name != "nvidia" or not torch.cuda.is_available():
        return False
    major, _ = torch.cuda.get_device_capability()
    return major == 9 and tuple(map(int, triton.__version__.split(".")[:2])) >= (3, 6)
...
def _gems_einsum_bf16_wrapper(x, xs, y, ys, x_bf16, y_bf16):
    return flag_gems.fp8_einsum("bhr,hdr->bhd", x_bf16, None, y_bf16, None)
